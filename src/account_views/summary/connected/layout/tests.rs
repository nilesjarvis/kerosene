...
fn connected_summary_compaction_keeps_full_actions_at_breakpoint() {
    ...
    assert!(!compaction.hide_sound());
    assert!(!compaction.hide_notifications());
...
fn connected_summary_compaction_hides_actions_by_priority() {
    ...
    let compaction = ConnectedSummaryCompaction::for_width(1_019.0);
    assert!(compaction.hide_sound());
    assert!(!compaction.hide_notifications());

    let compaction = ConnectedSummaryCompaction::for_width(939.0);
    assert!(compaction.hide_notifications());
    assert!(!compaction.hide_margin_ratio());
}
