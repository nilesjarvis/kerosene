...
impl TradingTerminal {
    pub(super) fn view_open_order_row<'a>(
        ...
        let cancel_cell: Element<'_, Message> = if can_cancel && !is_outcome_order {
            button(
                text("Cancel")
                    .size(10)
                    .center()
...
fn format_open_order_price(limit_px: Option<f64>) -> String {
    limit_px
        .map(|limit_px| format_usd(&limit_px.to_string()))
...
mod tests {
    ...
    fn open_order_formatters_mark_invalid_values() {
        assert_eq!(format_open_order_price(Some(100.0)), "$100.00");
        assert_eq!(format_open_order_price(None), "Invalid data");
