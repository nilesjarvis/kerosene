...
impl TradingTerminal {
    ...
    pub(crate) fn scrub_muted_ticker_state(&mut self) -> Task<Message> {
        ...
        self.live_watchlist_history_loaded_at
            .retain(|symbol, _| !is_muted(symbol));
        self.all_mids.retain(|symbol, _| !is_muted(symbol));
        ...
        if let Some(data) = self.account_data.take() {
            self.account_data = Some(Self::filter_account_data_for_muted_tickers_with(
                &exchange_symbols,
                &muted_tickers,
                data,
            ));
        ...
        for state in self.wallet_detail_windows.values_mut() {
            if let Some(data) = state.data.take() {
                state.data = Some(Self::filter_wallet_details_for_muted_tickers_with(
                    &exchange_symbols,
                    &muted_tickers,
                    data,
                ));
        ...
        for instance in self.charts.values_mut() {
            if !instance.symbol.is_empty() && is_muted(&instance.symbol) {
        ...
        if !self.active_symbol.is_empty() && is_muted(&self.active_symbol) {
            ...
            self.order_status = Some(("All available tickers are muted".into(), true));
        }
