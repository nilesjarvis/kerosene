...
#[test]
fn same_coin_book_updates_with_different_echoed_precision_do_not_collapse() {
    let (tx, mut rx) = broadcast::channel(16);
    let mut sender = CoalescedSender::with_interval(tx, Duration::from_millis(200));

    sender.submit(
        "l2Book".to_string(),
        Arc::new(json!({ "coin": "BTC", "nSigFigs": 5, "mantissa": 1, "seq": 1 })),
    );
    sender.submit(
        "l2Book".to_string(),
        Arc::new(json!({ "coin": "BTC", "nSigFigs": 4, "mantissa": 1, "seq": 2 })),
    );
    sender.submit(
        "l2Book".to_string(),
        Arc::new(json!({ "coin": "BTC", "nSigFigs": 5, "mantissa": 1, "seq": 3 })),
    );

    let drained_before = drain(&mut rx);
    assert_eq!(drained_before.len(), 2);
    assert_eq!(
        drained_before[0].1.get("seq").and_then(|v| v.as_i64()),
        Some(1)
    );
    assert_eq!(
        drained_before[1].1.get("seq").and_then(|v| v.as_i64()),
        Some(2)
    );
