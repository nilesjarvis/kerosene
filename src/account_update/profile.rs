...
mod tests {
    ...
    fn account(secret_id: &str, name: &str, wallet_address: &str) -> config::AccountProfile {
    ...
    fn chase_order(account_address: &str) -> ChaseOrder {
        ChaseOrder {
            ...
            agent_key: sensitive_string("account-a-agent-key"),
            is_buy: true,
            target_size: 1.0,
            filled_size: 0.0,
            remaining_size: 1.0,
    ...
    #[test]
    fn add_account_is_blocked_before_mutation_while_chase_order_is_active() {
        ...
        terminal.last_persisted_active_account_secret_id = Some("account-a".to_string());
        ...
        let toast = terminal.toasts.last().expect("blocked add should toast");
